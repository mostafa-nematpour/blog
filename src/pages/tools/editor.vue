...
<template>
    <div class="bg-bg-color-3 fixed w-full h-full top-0 bottom-0 right-0 left-0 inline-block font-sahel">
        <div class="h-full flex gap-1 flex-col">
            <div
                ...
                <button @click="copyHtmlToClipboard" class="bg-bg-color-3 text-slate-200 rounded px-1 font-semibold">copy
                    html</button>
            </div>
            <div v-if="showHTMLFlag == 'html'" v-html="html" :dir="dirRTL ? 'rtl' : 'ltr'"
            ...
            <div v-else-if="showHTMLFlag == 'code'"
                class="bg-bg-color-2 overflow-y-auto overflow-x-hidden max-w-[760px] w-full flex-1 m-auto relative rounded-[4px] px-2 py-3 blog-content">
                <textarea :value="html" class="w-full h-full" type="text" />
            </div>
            <div class="bg-bg-color-2 max-w-[760px] m-auto flex-1 w-full relative rounded-[4px] px-2 py-3 flex">
